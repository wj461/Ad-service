CREATE or REPLACE FUNCTION search_ads(age INTEGER, gender gender_type, country country_code, platform platform_type , offset_val INTEGER default 0, limit_val INTEGER default 5)
RETURNS TABLE (
    ad_id INTEGER,
    title TEXT,
    end_at TIMESTAMP
...
$$
    ...
    BEGIN
        ...
        IF gender IS NOT NULL THEN
            sql_statement := sql_statement || ' AND (gender = ' ||  quote_literal(gender) || ' OR gender IS NULL) ';
        END IF;
